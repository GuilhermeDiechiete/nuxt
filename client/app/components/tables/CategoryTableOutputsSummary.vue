...
const totalsByMonth = computed<TotalByMonth>(() => {
  return reportStore.totalOutputsByMonth as TotalByMonth
})

const data = computed<Categories[]>(() => {
  return reportStore.outputsCategoriesSummary as Categories[]
})
...
const columns: TableColumn<Categories>[] = [
...

  {
    ...
    meta: {
      class: {
        th: 'text-right',
        td: 'text-right font-bold text-error',
      }
    },
    cell: ({ row }) => formatCurrency(row.original.totalYear),
    footer: () => formatCurrency(reportStore.totalOutputsYear)
  }
]
</script>
